...
import { sortedProfiles, workerEnvironmentLabel } from "@/worker-environment";
...
export function WorkersPage({ workers, loaded, error }) {
  return <Page title="Workers" description="The machines available to pick up and execute work.">{error && <Failure value={error} />}{!loaded && !error ? <Loading description="Checking live worker status." /> : loaded && (workers.length ? <Card className="overflow-hidden">{workers.map((worker) => <article key={worker.instance_id} className="grid gap-4 border-b border-border p-4 last:border-b-0 sm:grid-cols-[minmax(12rem,1fr)_minmax(12rem,1fr)_10rem] sm:items-center sm:px-5">
    <div className="min-w-0"><div className="flex items-center gap-2"><Server className="size-4 text-muted-foreground" /><h2 className="truncate text-sm font-medium">{worker.name}</h2></div><p className="mt-1 truncate font-mono text-xs text-muted-foreground">{worker.instance_id}</p><p className="mt-1 truncate text-xs text-muted-foreground" title={workerEnvironmentLabel(worker.environment)}>{workerEnvironmentLabel(worker.environment)}</p></div>
    <div className="space-y-2"><div className="flex flex-wrap gap-1.5">{worker.repositories?.length ? worker.repositories.map((repository) => <Badge key={repository} className="border-border bg-muted font-mono text-muted-foreground">{repository}</Badge>) : <span className="text-xs text-muted-foreground">No repositories</span>}</div><div className="flex flex-wrap gap-1.5">{worker.transports?.map((transport) => <Badge key={transport} className="border-primary/25 bg-primary/10 font-mono text-primary">{transport}</Badge>)}{sortedProfiles(worker.profiles).map(([name, profile]) => <Badge key={name} title={[profile.harness, profile.provider, profile.reason].filter(Boolean).join(" · ")} className={profile.available ? "border-success/25 bg-success/10 font-mono text-success" : "border-danger/25 bg-danger/10 font-mono text-danger"}>{name}</Badge>)}</div>{worker.environment?.tags?.length ? <p className="text-xs text-muted-foreground">{worker.environment.tags.join(" · ")}</p> : null}</div>
...
export function CommandsPage() {
  ...
  return <Page title="Commands" description="Reusable instructions, tools, and execution constraints.">{definitions.loading ? <Loading /> : definitions.error ? <Failure value={definitions.error} /> : definitions.value.commands.length ? <div className="space-y-3">{definitions.value.commands.map((command) => <Card key={command.name} className="overflow-hidden">
