...
func TestGitHubCLIRejectsRequestWithoutAddingQueuedLabel(t *testing.T) {
	cli, runner := newScriptedGitHubCLI(
		scriptedGitHubResult{},
	)
	...
	if got := strings.Join(runner.calls[0], " "); !strings.Contains(got, "--remove-label machinist:requested") || strings.Contains(got, "--add-label") {
		t.Fatalf("rejected request acknowledgement = %s", got)
	}
}
